...
Method: original model, based on two bases, the two units with max signal strength are selected, but also for the triangulation using the min/max values (all combinations)
*/
...
function Triangulate($data) {
	...
	{
		if ($d != $maxi && $d != $nexti) $otherdata[] = $data[$d];
	}
	
	$combinations = array();
	for ($i=0;$i<count($otherdata)+2;$i++) $combinations[] = array(/*0 => 0,*/ 1 => 0, 2 => 0);
	$combinations = GetAllCombinations($combinations); //warning: huge growth!
	
	foreach ($combinations as $combination)
	{
		$data1a = array($data1[0], $data1[1], $data1[2 + $combination[0][1]]);
		$data2a = array($data2[0], $data2[1], $data2[2 + $combination[1][1]]);
		$otherdataa = array();
		foreach ($otherdata as $i=>$odata)
		{
			$otherdataa[] = array($odata[0], $odata[1], $odata[2 + $combination[$i+2][1]]);
		}
		list($result, $base) = DoTriangulate(array($data1a, $data2a, $otherdata), $factor, $roundto, $margin, $maxdis);
		if ($result !== false)
		{
	...
	{
		$results[$i] = array($subresults);
		/*foreach ($subresults as $a=>$result)
		{
			$subresults[$a] = array($result);
		}*/
	}
	$subresults = MergeSimilar($results, $margin);
	$results = array();
	foreach ($subresults as $subresult) $results[] = $subresult[0];
	...
	for ($i=0;$i<count($results);$i++)
	{
		...
		for ($a=0;$a<count($results);$a++)
		{
			if ($a != $i)
			{
				$d += sqrt(pow($results[$i][0] - $results[$a][0], 2) + pow($results[$i][1] - $results[$a][1], 2));
			}
		}
		if ($d < $mindis || $mindis === false)
		{
			$mindis = $d;
			$mini = $i;
		}
	...
	$solution = GetSolution($results, $maxdis, $factor);
