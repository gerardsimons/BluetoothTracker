...
public class ListActivity extends Activity {
    ...
    public void login(final String email, final String pass) {
        client.get(BASE_URL + "function=login&input=" + email + "," + pass, null, new TextHttpResponseHandler() {
            ...
            @Override
            public void onFailure(int StatusCode, org.apache.http.Header[] headers, String responseBody, java.lang.Throwable error) {
    ...
    public void addDevices(JSONObject json, String email, String pass) {
		...
		/*org.json.JSONArray jArray = json.names();
		for (int i = 0; i < jArray.length(); i++) {
			try {
				String name = jArray.getString(i);
				turflists.add(new TurfBlad(Integer.parseInt(name), json.getString(name), activity));
			} catch (JSONException e) {
				e.printStackTrace();
			}
		}*/
