...
public class CompassView extends View {
	...
	if (isCalibrated) {
		...
		double x = Math.cos((i * fragmentSizeRadians + nextRot) / 2D + rotationRadians) * radius;
		double y = Math.sin((i * fragmentSizeRadians + nextRot) / 2D + rotationRadians) * radius;

		paint.setARGB(255, fragmentColors[i][0], fragmentColors[i][1], fragmentColors[i][2]);
		paint.setStyle(Style.STROKE);
		canvas.drawArc(rectangle, (float) (i * fragmentSize + rotation), fragmentSize * 1.01f, false, paint);
		...
		canvas.drawText("" + fragments.get(i).getRepresentativeValue(), (float) x + cx - center, (float) y + cy + center, paint);
	    }
		...
		int[] color = interpolateColors(i / (double) nrOfFragments, new int[] { 50, 50, 50 }, new int[] { 255, 255, 255 });

		//Log.d(TAG, "Color #" + i + " = " + Utilities.arrayToString(color));
		...
		int calibrationValuesLeft = fragments.get(i).valuesLeftForCalibration();
		if (calibrationValuesLeft > 0) {
		    //		    canvas.drawText("" + fragments.get(i).valuesLeftForCalibration(), (float) x + cx - center, (float) y + cy + center, paint);
		    canvas.drawText("" + calibrationValuesLeft, (float) x + cx - center, (float) y + cy + center, paint);
		} else {
		    paint.setColor(Color.GREEN);
		    canvas.drawText("" + fragment.getRepresentativeValue(), (float) x + cx - center, (float) y + cy + center, paint);
		}
	    }
	}
	...
	paint.setColor(Color.BLACK);
	paint.setTextSize(50);
	//String azimuth = new BigDecimal(String.valueOf(rotation)).setScale(5, BigDecimal.ROUND_HALF_UP).toString();
	...
	if (fragments != null) {
		...
		double ratio = (fragments.get(i).getRepresentativeValue() - minRSSI) / delta;
	...
	if (colorOne.length == colorTwo.length) {
		...
		int newChannelValue = (int) Math.round((1 - ratio) * colorTwo[i] + ratio * colorOne[i]);
